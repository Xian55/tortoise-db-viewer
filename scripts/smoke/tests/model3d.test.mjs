...
import { afterRemount, viewerIdle, waitStable } from "./_shared.mjs";
...
async function testIdleCostsNothing(id) {
  ...
  // Two different waits, and only the second is really a wait. "The loop stopped" is a
  // state the viewer reports, so wait for it instead of sleeping 600ms hoping it happened.
  // The window after it is a genuine OBSERVATION -- the whole assertion is that the frame
  // counter does not move -- but it only has to be long enough to have caught a running
  // loop, which at 30fps is ~9 frames rather than the 36 the old 1200ms watched for.
  await page.waitForFunction(() => window.__mv().running === false, { timeout: T });
...
async function testViewControls() {
  ...
  await page.waitForFunction(() => window.__mv && window.__mv().triangles > 0, { timeout: T });
  // Wait for the state being asserted, never for a guessed number of milliseconds. The
  // viewer already reports `running`/`spinning`, so "the room opened still" is a thing to
  // wait FOR -- which is both ~2.4s faster here and stricter, since a fixed sleep passes
  // whenever the loop happens to be between frames at the moment it expires.
  ...
  await page.waitForFunction(() => window.__mv().spinning === true, { timeout: T });
  ...
  await page.click("#dress-spin");
  await page.waitForFunction(() => window.__mv().spinning === false, { timeout: T });
...
async function testLocks() {
  ...
  // Each roll re-equips, which rebuilds the viewer -- so wait for that rather than for a
  // guessed 700ms, six times over. This one loop was 4.2s of the suite's wall floor.
  for (let i = 0; i < 6; i++) await afterRemount(() => page.click("#dress-item"));
  const after = await page.evaluate(() => {
...
async function testFullscreen() {
  ...
  await page.waitForFunction(() => !!document.fullscreenElement, { timeout: T }).catch(() => {});
  const during = await waitStable(box) || await box();
  await page.evaluate(() => document.exitFullscreen()).catch(() => {});
  await page.waitForFunction(() => !document.fullscreenElement, { timeout: T }).catch(() => {});
  const after = await waitStable(box) || await box();
...
async function testResetAndSlotArt() {
  ...
  await waitStable(() => page.evaluate(() => window.__mv.view()));   // damping has to settle
...
async function testPickerPlacement() {
  ...
  const open = async (slot, term) => {
    ...
    await page.waitForFunction(() => document.querySelector("#dress-pop")?.getBoundingClientRect().height > 0, { timeout: T });
    ...
    await page.waitForFunction(() => document.getElementById("dress-pop")?.hidden !== false, { timeout: T }).catch(() => {});
