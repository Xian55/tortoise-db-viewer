...
scripts/data/custom-display.json      committed: display_id -> custom icon name
...
- `scripts/extract-icons.py` — LOCAL: pulls Turtle custom BLP icons from the
  client MPQs (StormLib) → `assets/icons/custom/*.webp` + `scripts/data/
  custom-display.json` (the `display_id → icon` supplement build-db merges).
...
- **Don't commit built data.** It's regenerated by CI from the server repo. The
  one exception: custom icons (`assets/icons/custom/`, `public/icons/custom-atlas.*`,
  `scripts/data/custom-display.json`) — their source is the game client, which CI
  can't reach, so they're committed (see "Custom icons").
